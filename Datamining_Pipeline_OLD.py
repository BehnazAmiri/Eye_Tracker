...
def get_input_range():
    ...
    tk.Checkbutton(root, text="AOI Time Summary per Question (New)", variable=plot_vars["aoi_summary_per_question"]).pack(anchor='w', padx=20, pady=2)
...
def clean_and_prepare_data(df, invalid_gaze_threshold):
    """
    ...
    # Create a validity flag for each raw gaze sample
    df['raw_valid_gaze_sample'] = (df.get('BPOGV', 1) == 1) & (df[gaze_x_col] > 0) & (df[gaze_y_col] > 0)

    # Calculate the number of invalid gaze samples per participant-question
    ...
    # Print the invalid gaze stats for debugging
    print("Gaze Validity Stats:")
    print(gaze_validity_stats)

    # Identify questions to remove based on the threshold
    questions_to_remove = gaze_validity_stats[
        gaze_validity_stats['invalid_gaze_ratio'] > invalid_gaze_threshold
    ][['participant_id', 'question_id']]

    # Filter out questions with high invalid gaze ratio in a memory-efficient way
    if not questions_to_remove.empty:
        # Using an anti-join with 'isin' on a MultiIndex is more memory-efficient than a merge
        ...
        df = df[~df_idx.isin(keys_to_remove_idx)].copy()

    # Now filter out invalid gaze data for the remaining valid questions
    df = df[df['raw_valid_gaze_sample']].copy()
    df[timestamp_col] = pd.to_numeric(df[timestamp_col], errors='coerce')
    df.dropna(subset=[timestamp_col], inplace=True)
    df.sort_values(by=['participant_id', 'question_id', timestamp_col], inplace=True)
    
    # Calculate total time (t_ij) per question for each participant, preserving part info
    time_df = df.groupby(['participant_id', 'question_id', 'part'])[timestamp_col].agg(['min', 'max']).reset_index()
...
def detect_outliers(df, time_cap_s=None, exclude_censored=False):
    """
    ...
    if base.empty:
        ...
        stats_per_question['LB'] = stats_per_question['Q1'] - (1.5 * stats_per_question['IQR'])
...
def apply_behavioral_labels(df, answer_data, time_cap_s=None, exclude_censored=False):
    """
    ...
    if correct_answers_df.empty:
        ...
        correct_stats['UF_C'] = correct_stats['Q3_C'] + (1.5 * correct_stats['IQR_C'])
...
def main():
    ...
    def pipeline_logic(progress_queue, cancel_event):
        ...
        try:
            ...
            time_df = clean_and_prepare_data(raw_df.copy(), invalid_gaze_threshold=invalid_gaze_threshold)
            ...
            outlier_df, stats_all = detect_outliers(time_df.copy(), time_cap_s=time_cap_s, exclude_censored=exclude_censored)
            ...
            labeled_df, stats_c = apply_behavioral_labels(outlier_df.copy(), answer_data.copy(), time_cap_s=time_cap_s, exclude_censored=exclude_censored)
