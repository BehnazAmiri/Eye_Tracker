...
# Load configuration from module-scoped config.ini (relative to this file)
...
CONFIG = {
    'SCREEN': {
        k: config.getboolean('SCREEN', k) if k in ('fullscreen', 'topmost')
        else config.getint('SCREEN', k)
    ...
    'EYE_TRACKER': {
        ...
        # commands in config.ini: comma-separated, without "\r\n"
        'commands': [cmd.strip() for cmd in config.get('EYE_TRACKER', 'commands').split(',')],
    },
    'DIMENSIONS': {
        k: config.getfloat('DIMENSIONS', k) if 'ratio' in k else config.getint('DIMENSIONS', k)
        for k in config['DIMENSIONS']
    },
    'COLORS': dict(config['COLORS']),
    'FONTS': {
        k: (
            config.getint('FONTS', k) if 'size' in k and 'ratio' not in k else
            config.getfloat('FONTS', k) if 'ratio' in k else
            config.get('FONTS', k)
        )
...
def _resolve_module_path(path_value: str) -> str:
    if not path_value:
        return path_value
    if os.path.isabs(path_value):
        return path_value
    return os.path.normpath(os.path.join(_SCRIPT_DIR, path_value))


# Normalize module paths so the app does not depend on the current working directory.
for _k in ("questions_json", "question_exams_dir", "output_dir"):
    if _k in CONFIG.get("PATHS", {}):
        CONFIG["PATHS"][_k] = _resolve_module_path(CONFIG["PATHS"][_k])
...
class EyeTrackerClient:
    ...
    def connect(self):
        ...
        try:
            ...
            self.socket.connect(
                (CONFIG['EYE_TRACKER']['host'], CONFIG['EYE_TRACKER']['port'])
            )
            # Send each command with CR+LF appended
...
class QuestionDisplayApp:
    ...
    def _build_ui(self):
        ...
        self.question_font_size = max(
            int(((self.left_width + self.screen_height) / 2) * CONFIG['FONTS']['question_size_ratio']), 8)
        self._create_panels()
    ...
    def _configure_sizes(self):
        ...
        self.option_font_size = max(
            int(((self.left_width + self.screen_height) / 2) * CONFIG['FONTS']['option_size_ratio']), 8)
    ...
    def _load_questions(self):
        ...
        try:
            participant_file = os.path.join(CONFIG['PATHS']['question_exams_dir'],
                                            f"Participant_{self.participant_id}.json")
    ...
    def _select_option(self, option_id, index):
        ...
        self.options_inner.winfo_children()[index].config(fg=CONFIG['COLORS']['selected_fg'],
    ...
    def _blink_timer(self):
        if self.is_blinking:
            color = CONFIG['COLORS']['question_bg'] if self.timer_label.cget("fg") == CONFIG['COLORS']['timer'] else \
            CONFIG['COLORS']['timer']
